...
class OrderApp:
    ...
    def _refresh_data(self):
        ...
        self.customer_model.load_data()
        self.product_model.load_data()
        self.order_model.load_data()
    ...
    def import_products(self):
        ...
        if file_path:
            try:
                ...
                self.product_model.load_data()  # Перезагружаем данные из файла
                self.filtered_products = self.product_model.get_all().copy()  # Обновляем фильтрованный список
                self.update_product_listbox()  # Обновляем список товаров на первой вкладке
    ...
    def import_customers(self):
        ...
        if file_path:
            try:
                ...
                self.customer_model.load_data()  # Перезагружаем данные из файла
                self.filtered_customers = self.customer_model.get_all().copy()  # Обновляем фильтрованный список
                self.update_customer_listbox()  # Обновляем список покупателей на первой вкладке
